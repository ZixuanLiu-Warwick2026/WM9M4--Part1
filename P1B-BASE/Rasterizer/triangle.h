...
public:
    ...
    void draw(Renderer& renderer, Light& L, float ka, float kd) {
        ...
        for (int y = yStart; y < yEnd; y++) {
...
#if defined(__AVX2__)
            // ---- AVX2 block: process 8 pixels per step ----
            for (; x + 7 < xEnd; x += 8) {
                ...
                if (insideMaskBits != 0) {
                    ...
                    // per-lane scalar commit (minimal change, safe)
                ...
                // advance E scalars by 8 pixels: E += dx * 8
                E01 += e01_dx * 8.f;
                E12 += e12_dx * 8.f;
                E20 += e20_dx * 8.f;
            }
            ...
            // ---- tail scalar pixels (and also whole loop if no AVX2) ----
            for (; x < xEnd; x++) {

...
// ✅【新增】std::thread / join 用
#include <thread>

// ✅【新增】虽然你文件里已经在用 vector（可能被别的头带进来了），但建议显式 include，稳
#include <vector>

// ✅【新增】用于 std::cin.ignore 清理输入缓冲
#include <limits>
...
// ===============================================================
// ✅【新增】多线程渲染 worker：只渲染 [yMin, yMax) 的屏幕行
// 放置位置：紧跟在 render(...) 函数后面
//
// 关键点：
// 1) 每个线程只写自己负责的 y 范围 -> 不需要 mutex/atomic
// 2) Light 不能共享引用（triangle.draw 内会 normalise，会写 L）
//    所以这里对 L 做线程局部拷贝：Light Llocal = L;
// ===============================================================
static void renderSlice(Renderer& renderer,
    Mesh* mesh,
    matrix& camera,
    Light& L,
    int yMin,
    int yMax)
{
    // ✅【新增】每线程拷贝一份 Light，避免多线程同时写 L.omega_i 造成数据竞争
    Light Llocal = L;
    ...
    for (triIndices& ind : mesh->triangles) {
        ...
        // ✅【关键】把线程负责的 y 范围传进 tri.draw（triangle.h 里你已经加了 yClip）
        triangle tri(t[0], t[1], t[2]);
        tri.draw(renderer, Llocal, mesh->ka, mesh->kd, yMin, yMax);
    }
}

// ===============================================================
// ✅【新增】renderMT：按屏幕高度把工作切成 numThreads 段，用 std::thread 并行
// 放置位置：紧跟在 renderSlice(...) 后面
//
// 行为：
// - numThreads: 用户输入 1~11（这里再做一次 clamp 防呆）
// - 每个线程处理一段 scanlines: [y0, y1)
// - 最后 join 等所有线程结束（课件重点）
// ===============================================================
void renderMT(Renderer& renderer, Mesh* mesh, matrix& camera, Light& L, int numThreads)
{
    // ✅【新增】防呆：线程数限制 1~11（你说用户输入 1-11 任意都可以）
    numThreads = std::max(1, std::min(11, numThreads));
    ...
    // ✅【新增】如果只用 1 线程，直接走原 render（方便对照测试）
    if (numThreads == 1) {
        render(renderer, mesh, camera, L);
        return;
    }
    ...
    // 按高度平均切分，每个线程一段 y
    const int work = H / numThreads;

    for (int i = 0; i < numThreads; i++) {
        const int y0 = i * work;
        const int y1 = (i == numThreads - 1) ? H : (i + 1) * work; // 最后一个线程吃掉余数

        // ✅【新增】按课件标准写法：创建 thread，最后统一 join
        // 注意：renderer / camera / L 传引用，要用 std::ref
        threads[i] = std::thread(renderSlice,
            std::ref(renderer),
            mesh,
            std::ref(camera),
            std::ref(L),
    ...
    // ✅【新增】join：主线程等待所有子线程完成（课件强调必须 join）
    for (auto& t : threads)
        t.join();
}
...
// ===============================================================
// ✅【新增 / 改法 A】多线程渲染整个 scene：每帧只创建一次线程
// 关键点：
// 1) 不再 “每个 mesh 调一次 renderMT -> 每次都创建 numThreads 个线程”
// 2) 改成 “每帧只创建 numThreads 个线程”，每个线程负责一个 y slice
// 3) 每个线程在自己的 y slice 内，遍历 scene 里所有 mesh 并调用 tri.draw(..., yMin, yMax)
//
// 放置位置：建议放在 renderMT(...) 后面
// ===============================================================

// ✅【新增】scene 的 worker：渲染 scene 中所有 mesh，但只写 [yMin, yMax)
static void renderSceneSlice(Renderer& renderer,
    ...
    int yMax)
{
    // ✅ 每线程拷贝 Light，避免 triangle.draw 内部 normalise 导致共享写入
    Light Llocal = L;
    ...
    for (Mesh* mesh : scene) {
        ...
        for (triIndices& ind : mesh->triangles) {
            ...
            // ✅ 只渲染本线程负责的 y 范围（triangle.h 的 yClip）
            triangle tri(t[0], t[1], t[2]);
            tri.draw(renderer, Llocal, mesh->ka, mesh->kd, yMin, yMax);
        }
    }
...
// ✅【新增】每帧只创建一次线程的 renderSceneMT
void renderSceneMT(Renderer& renderer,
    const std::vector<Mesh*>& scene,
    matrix& camera,
    Light& L,
    int numThreads)
{
    ...
    for (int i = 0; i < numThreads; i++) {
        const int y0 = i * work;
        const int y1 = (i == numThreads - 1) ? H : (i + 1) * work;

        threads[i] = std::thread(renderSceneSlice,
            std::ref(renderer),
            std::cref(scene),   // ✅ 只读引用（scene 容器不修改）
            std::ref(camera),
            std::ref(L),
            y0,
            y1);
...
// ===============================================================
// ✅【新增】只询问一次线程数：允许用户输入 1~11 任意整数
// 放置位置：任意 scene 函数之前（建议：renderMT(...) 后、sceneTest() 前）
//
// 行为：
// - 第一次调用会提示输入
// - 后续调用直接返回同一个值（不会重复问）
// - 非数字/非法输入：使用默认值 defaultThreads
// - 超范围：clamp 到 [1, 11]
// ===============================================================
static int getUserThreadCount()
{
    static bool initialised = false;
    static int cachedThreads = 4; // ✅默认线程数：4（你想默认 1 也可以改这里）

    if (initialised) return cachedThreads;
    initialised = true;

    std::cout << "\n====================================\n";
    std::cout << "Please input thread count (1 ~ 11):\n";
    std::cout << "====================================\n> ";
    ...
    if (!(std::cin >> t)) {
        ...
        std::cout << "[Warn] Invalid input. Use default " << cachedThreads << " threads.\n\n";
    ...
    // ✅ clamp 到 1~11（你说 1-11 任意都可输入）
    if (t < 1) t = 1;
    if (t > 11) t = 11;

    cachedThreads = t;
    std::cout << "[Info] Using " << cachedThreads << " threads.\n\n";
    return cachedThreads;
}
...
void sceneTest() {
   ...
   // scene.push_back(&mesh2); 
    ...
    // Main rendering loop
    while (running) {
        const int numThreads = getUserThreadCount(); // ✅【新增】只问一次
     ...
     //   mesh2.world = matrix::makeTranslation(x, y, z) * matrix::makeRotateX(0.01f);
        ...
        renderSceneMT(renderer, scene, camera, L, numThreads);
...
void scene1() {
    ...
    while (running) {
        ...
        // ✅【新增】获取线程数（只会提示一次）
        const int numThreads = getUserThreadCount();
...
void scene3() {
    ...
    while (running) {
        ...
        // ✅ [Added] Get the number of threads (prompted only once)
        const int numThreads = getUserThreadCount();
        ...
        // Print the total time cost for this segment every 60 frames
        // (a larger value indicates slower performance)
        if (++cycle % 120 == 0) {
            end = std::chrono::high_resolution_clock::now();
            std::cout << (cycle / 120) << " :"

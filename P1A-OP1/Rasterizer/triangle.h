...
public:
    ...
    void draw(Renderer& renderer, Light& L, float ka, float kd) {
        ...
        // Optimisation 1: precompute loop bounds (avoid repeated ceil in loops)
        const int yStart = (int)(minV.y);
        const int yEnd = (int)ceil(maxV.y);
        const int xStart = (int)(minV.x);
        const int xEnd = (int)ceil(maxV.x);

        // Optimisation 1: normalise light direction once per draw() call, not per pixel
        L.omega_i.normalise();
        ...
        for (int y = yStart; y < yEnd; y++) {
            for (int x = xStart; x < xEnd; x++) {
                ...
                // inside test (barycentric)
                if (getCoordinates(vec2D((float)x, (float)y), alpha, beta, gamma)) {

                    // Optimisation 1: depth first -> z-test first -> only then compute normal/colour
                    float depth = interpolate(beta, gamma, alpha, v[0].p[2], v[1].p[2], v[2].p[2]);

                    // Optimisation 1: read zbuffer once, reuse (avoid duplicate access)
                    float& z = renderer.zbuffer(x, y);

                    // Early reject: if fails z-test or invalid depth, skip expensive work
                    if (!(z > depth && depth > 0.001f))
                        continue;

                    // Only now do more expensive interpolations
                    ...
                    // shading (now uses pre-normalised L.omega_i)
